...
#End Region
#Region "Columns Edit"
    Dim EditMa_BP_I, EditMa_HS_I, editma_chucvu, editma_chucdanh, editis_nghi, EditTien_HH, editTham_nien, edithe_so_TT, editTien_thuong, editTy_le, editKPIs, EditHe_so_thuong, editDiem_thuong, Editma_vt As New Cyber.Fill.CyberColumnGridView
#End Region
...
#Region "Load And Set And Default"
    ...
    '    Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GETCTBP_HHDV", M_Stt_Rec & "#" & TxtNgay_Ct.Value & "#" & cmbNam.SelectedValue & "#" & cmdthang.SelectedValue & "#" & M_Ma_CT & "#" & M_Mode & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Private Sub V_GetColumn()
        ...
        editTham_nien.GetColumn(DetailGRV, "Tham_nien")
        'editngay_lv.GetColumn(DetailGRV, "Ngay_lv")
        EditHe_so_thuong.GetColumn(DetailGRV, "he_so_thuong")
        edithe_so_TT.GetColumn(DetailGRV, "he_so_tt")
        editTy_le.GetColumn(DetailGRV, "Ty_le")
        editKPIs.GetColumn(DetailGRV, "KPIs")
        editDiem_thuong.GetColumn(DetailGRV, "Diem_thuong")
        editTien_thuong.GetColumn(DetailGRV, "Tien_thuong")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        'If Not EditTien_HH Is Nothing Then AddHandler EditTien_HH.EditColumn.Leave, AddressOf L_Tinh_Toan
        'If Not editTien_thuong.Column Is Nothing Then AddHandler editTien_thuong.EditColumn.Leave, AddressOf L_Tinh_Toan
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        ...
        '    Dv_DetailTmp.Item(iRow).Item("he_so_thuong") = Dv_DetailTmp.Item(iRow).Item("He_so_TT") * Dv_DetailTmp.Item(iRow).Item("TY_LE") * Dv_DetailTmp.Item(iRow).Item("KPIs") / 100 + Dv_DetailTmp.Item(iRow).Item("Diem_thuong")
    ...
    Private Sub UpdateList2()
        ...
        '    If TxtT_he_so.Double = 0 Then
        '        TxtT_he_so.Double = 1

        '    End If
        '    Dv_DetailTmp.Item(iRow).BeginEdit()
        '    Dv_DetailTmp.Item(iRow).Item("tien_thuong") = TxTT_Tien_thuong.Double / TxtT_he_so.Double * Dv_DetailTmp.Item(iRow).Item("he_so_thuong")
        '    Dv_DetailTmp.Item(iRow).EndEdit()
        'Next
    ...
    Private Sub UpdateList1()
        'Dim n_tONG As Decimal = 0

        'For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
        '    n_tONG = n_tONG + Dv_DetailTmp.Item(iRow).Item("HE_SO_THUONG")

        'Next

        'TxtT_he_so.Double = n_tONG

    End Sub
    Private Sub UpdateList3()
        'Dim n_tONG_KPI As Decimal = 0

        'For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
        '    n_tONG_KPI = n_tONG_KPI + Dv_DetailTmp.Item(iRow).Item("Tien_thuong")

        'Next

        'TxtT_tien_KPI.Double = n_tONG_KPI
...
#Region "Valid - Detail - Add On"
    ...
    Private Sub L_Ma_Vt(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        Dim _TK_VT As String = DrReturn.Item("Ten_vt")
        Dim _nh_VT As String = DrReturn.Item("Nh_vt1")


        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ten_vt") = IIf(Dv_DetailTmp.Item(iRow).Item("Ten_vt").ToString.Trim = "", _TK_VT, Dv_DetailTmp.Item(iRow).Item("Ten_vt").ToString.Trim)
        Dv_DetailTmp.Item(iRow).Item("Nh_KH1") = IIf(Dv_DetailTmp.Item(iRow).Item("Nh_KH1").ToString.Trim = "", _nh_VT, Dv_DetailTmp.Item(iRow).Item("Nh_KH1").ToString.Trim)
        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
    ...
    Private Sub V_Ma_chucdanh(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        CyberSupport.V_Lookup("Ma_chucdanh", "DMchucdanh", "1=1", "Ma_BP = '" + TxtMa_hs_h.Text.ToString.Trim + "'", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
