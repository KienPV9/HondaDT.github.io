...
#Region "Columns Edit"
    ...
    Dim Editso_luong1, Editdvt1, Editgia_nhap_QD, EditHe_so1 As New Cyber.Fill.CyberColumnGridView 'DevExpress.XtraEditors.Repository.RepositoryItemTextEdit
...
#End Region
    Private Sub PNK_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        '----------------------------------------------------------------------------------------------------------
        AddHandler TxtSo_PO_SAP.Leave, AddressOf L_So_PO_SAP
        ...
        AddHandler CmdLoc_POD.Click, AddressOf V_Loc_POD
        AddHandler CmdDanh_Sach_PO.Click, AddressOf V_Danh_sach_PO
    End Sub
    ...
    Private Sub VisibleColumnMa_GD()
        If CbbMa_GD.SelectedValue.ToString.ToUpper = "1" Then
            TxtLenh_PO.Enabled = True
            CmdLoc_POD.Enabled = False
        Else
            TxtLenh_PO.Enabled = False
            CmdLoc_POD.Enabled = True
        End If
    ...
    Private Sub V_GetNoVoucherAuto()
        ...
        'TxtMa_nt.Text = CyberSupport.V_GetNoVoucherAuto(M_Mode, TxtLenh_PO.Text, M_Stt_Rec, _Ngay_Ct, _Ngay_LCt, M_Ma_CT, M_Ma_Dvcs, M_User_Name, AppConn, CyberSmlib)
        Dim _DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetnoVoucherByNo", M_Mode + "#" + TxtLenh_PO.Text.Trim + "#" + M_Stt_Rec + "#" + _Ngay_Ct.ToString("yyyyMMdd") + "#" + _Ngay_LCt.ToString("yyyyMMdd") + "#" + M_Ma_GD + "#" + M_Ma_CT + "#" + M_Ma_Dvcs + "#" + M_User_Name)
        txtSo_ct.Text = _DsTmp.Tables(1).Rows(0).Item("So_CT").ToString
...
#Region "Nhập kho theo đề nghị"
    Private Sub L_So_DN(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        TxtSo_DN.Text = Sys.V_GetSo_DN("PDN", M_Ma_GD, M_Mode, _Dt, TxtSo_DN.Text, TxtMa_Dvcs.Text)
    ...
    Private Sub V_GetDetail(ByVal _So As String)
        ...
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_RoGetPNKFromPDN", _So & "#" & M_Stt_Rec & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        ...
        txtDien_giai.Text = DsTmp.Tables(2).Rows(0).Item("Dien_giai")
        '
...
#Region "Lenh/SO/Po/Ro/Vt"
    Private Sub L_Lenh_PO_Update(ByVal _Tb_Master As DataTable, ByVal _Dt_Detail As DataTable)
        ...
        UpdateList()
    End Sub
    Private Sub L_So_PO_SAP(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Or TxtSo_PO_SAP.Text.Trim = "" Then Exit Sub
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetSo_PO_SAP", TxtSo_PO_SAP.Text.Trim & "#" & M_Stt_Rec & "#" & M_Ma_CT & "#" & M_Mode & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        ...
        TxtLenh_PO.Text = DsTmp.Tables(1).Rows(0).Item("So_PO").ToString.Trim
        TxtCheck_PO.Text = TxtLenh_PO.Text
        L_Lenh_Po1(sender, e)
        V_Sua_Thue(sender, e)
    End Sub
    ...
    Private Sub L_Lenh_Po(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If TxtLenh_PO.Text.Trim = "" Then Return
        Dim _DsTmp As DataSet = CyberInput.V_GetPNKFromPOD(AppConn, CyberSmlib, "1", "19000101", "19000101", TxtLenh_PO.Text, TxtMa_Kh.Text, M_Stt_Rec, M_Ma_CT, M_Ma_Dvcs, M_User_Name)
        ...
        L_Lenh_PO_Update(_DsTmp.Tables(0), _DsTmp.Tables(2))
        _DsTmp.Dispose()
    End Sub
    Private Sub L_Lenh_Po1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim nCount As Integer = Dt_DetailTmp.Rows.Count - 1
        For iRow As Integer = nCount To 0 Step -1
        ...
        _DsTmp.Dispose()
    End Sub
    Private Sub V_Loc_POD(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim Frm As PNK_Loc
        Frm = New PNK_Loc
        ...
        L_Lenh_PO_Update(_Dt_Return1, _Dt_Return2)
    End Sub
...
#End Region
#Region "Xem lịch sử"
    Private Sub V_Danh_sach_PO(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim str As String
        Dim dt As Date = TxtNgay_Ct.Value
        str = CyberSupport.V_GetStrSelectList(AppConn, osysvar, M_Para, "CP_GetListPNKGetSo_Po", dt.ToString("yyyyMMdd") & "#" & TxtMa_Kh.Text & "#" & M_Ma_Dvcs & "#" & M_User_Name, M_LAN)
        If str Is Nothing Then Exit Sub
        TxtLenh_PO.Text = str.ToString.Trim
        L_Lenh_Po(sender, e)
    End Sub
#End Region
#End Region
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        If CbbMa_GD.SelectedValue = "2" Then
            ...
            If Math.Abs(_tien_sua_tay - Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("Gia_NT0")) > 10 Then
                ...
                Dv_DetailTmp.Item(iRow).Item("TIEN_NT0") = Dv_DetailTmp.Item(iRow).Item("So_Luong1") * Dv_DetailTmp.Item(iRow).Item("Gia_nhap_Qd")
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub L_Ma_Vt(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Dv_DetailTmp.Table.Columns.Contains("Ma_kho_I") And DrReturn.Table.Columns.Contains("Ma_kho_I") Then If Dv_DetailTmp.Item(iRow).Item("Ma_kho_I").ToString.Trim = "" Then Dv_DetailTmp.Item(iRow).Item("Ma_kho_I") = DrReturn.Item("Ma_Kho").ToString.Trim
        If Dv_DetailTmp.Table.Columns.Contains("Ma_Vitri") And DrReturn.Table.Columns.Contains("Ma_Vitri") Then If Dv_DetailTmp.Item(iRow).Item("Ma_Vitri").ToString.Trim = "" Then Dv_DetailTmp.Item(iRow).Item("Ma_Vitri") = DrReturn.Item("Ma_Vitri").ToString.Trim
...
#Region "Button"
    Private Sub V_Save(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim convertedDate As DateTime = Convert.ToDateTime(TxtNgay_Ct0.Value)

        If TxtSo_PO_SAP.Text = "" Then
            DetailGRV.PostEditor()
        ...
        Else
            If MsgBox("Ngày hóa đơn VAT là " + "ngày " & convertedDate.Day & "/ " & convertedDate.Month & "/ " & convertedDate.Year, MsgBoxStyle.YesNo, "Cyber - Honda Đồng Tháp") = MsgBoxResult.Yes Then
    ...
    Private Sub V_Save_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value
        Dim _Ngay_LCt As Date = TxtNgay_Ct.Value
        If CbbMa_GD.SelectedValue.ToString.Trim = "2" Then TxtLenh_PO.Text = ""
        '----------------------------------------------------------------------------------------------------------------------
        V_GetDateVoucher(M_Mode, _Ngay_Ct, _Ngay_LCt)
        '----------------------------------------------------------------------------------------------------------------------
        If Not ChkRightsVoucher("L") Then Exit Sub
