...
#Region "Columns Edit"
    ...
    Dim EditCl_Pb, EditTen_Tk_i, EditTen_kh_i, EditSo_hd, EditNgay_HD, EditTien_Hd, EditTien_Hd_NT, EditDa_PB, EditDa_PB_NT, EditCl_Pb_NT, EditTy_Gia_HT As New Cyber.Fill.CyberColumnGridView 'DevExpress.XtraEditors.Repository.RepositoryItemTextEdit
#End Region
#Region "Khai bao bien Private"
    Dim M_Ct, M_Ph As String
    Dim DrReturn As DataRow
    Dim M_Count As Integer = 1
    Dim DtPost, DtMaGD, DtMaTT, DmLoaiThu As DataTable
    Dim DSMaTT As New DataSet
    Dim M_Ma_TT As String = "C"
    Dim M_Loai_Thu As String = ""
#End Region
    Private Sub PT5_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Load And Set And Default"
    Private Sub V_Load()
        ...
        TxtT_TT_I_C_W_N.InputMask = IIf(DrDmct.Item("M_IP_MK_TIEN").ToString.Trim <> "", DrDmct.Item("M_IP_MK_TIEN").ToString.Trim, TxtT_TT_I_C_W_N.InputMask)
    End Sub
    Private Sub V_Getdefault()
        ...
        DSMaTT = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetLoaiTT", M_Ma_CT & "#" & M_User_Name)
        DtMaTT = DSMaTT.Tables(0)
        ...
        CyberFill.V_FillComBoxValue(Me.CbbMa_TT, DtMaTT, "Ma_TT", If(M_LAN = "V", "Ten_TT", "Ten_TT"), M_Ma_TT)

        DmLoaiThu = CyberSmodb.OpenTableKey(AppConn, Nothing, "DmLoaiThu", "Loai_Thu", "1=1", CyberSmlib)

        CyberFill.V_FillComBoxDefaul(Me.CbbLoai_Thu, DmLoaiThu, "Loai_Thu", If(M_LAN = "V", "Ten_Loai", "Ten_Loai2"), "Ngam_Dinh")

        Dim Drs As DataRow() = DmLoaiThu.Select("Ngam_Dinh = '1'")
        If Drs.Length > 0 Then M_Loai_Thu = Drs(0).Item("Loai_Thu").ToString.Trim
        '---------------------------------------------------------------------------------------------'
        V_LoadDefault()
        '---------------------------------------------------------------------------------------------'
    End Sub
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_System()
        ...
        ' ngam dinh tk thu tien
        AddHandler CbbLoai_Thu.SelectedValueChanged, AddressOf L_Loai_Thu
        '----------------------------------------------------------------------------------------------------------
        AddHandler CmdXe_Ra.Click, AddressOf V_Xe_Ra
        AddHandler CmdHoa_Don.Click, AddressOf V_Hoa_Don
    ...
    Private Sub VisibleColumnMa_NT()
        If TxtMa_NT.Text.Trim = "" Or TxtMa_NT.Text.Trim.ToUpper = osysvar("M_Ma_NT0").ToString.Trim.ToUpper Then
            ...
            EditTy_Gia_HT.Visible = False
            TxtT_TT.Visible = False
        Else
            EditTien.Visible = True
            EditTy_Gia_HT.Visible = True
    ...
    Private Sub V_Visible_PB()
        ...
        'If Not ColTy_Gia_HT Is Nothing Then ColTy_Gia_HT.Visible = _V_NT
        ...
        If (M_Mode = "M" Or M_Mode = "S") Then
            ...
            If _V Then EditTy_Gia_HT.ColumnReadOnly = True Else EditTy_Gia_HT.ColumnReadOnly = False
        End If
    End Sub
    ...
    Private Sub L_Ma_GD(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        VisibleColumnMa_GD()
    End Sub
    Private Sub L_Loai_Thu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim tk_NgamDinh As String = ""
        tk_NgamDinh = CyberSmodb.SQLGetvalue(AppConn, "Tk_NgamDinh", "DmLoaiThu", "loai_thu = N'" + CbbLoai_Thu.SelectedValue.ToString.Trim + "'", CyberSmlib)
        If tk_NgamDinh = "" Then tk_NgamDinh = "1131"
        TxtTk.Text = tk_NgamDinh
        Txtten_Tk.Text = CyberSmodb.SQLGetvalue(AppConn, "Ten_TK", "dmtk", "tk = N'" + tk_NgamDinh + "'", CyberSmlib)
    End Sub
#End Region
#Region "Valid - Master"
...
#Region "Valid --- Tao phieu Xe Ra - Hoa don"
    ...
    End Sub
    Private Sub V_Hoa_Don(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ''If (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If Dt_DetailTmp.Rows.Count <= 0 Then Exit Sub
        If Not Dt_DetailTmp.Columns.Contains("So_RO") Then Exit Sub
        Dim _So_Ro As String = Dt_DetailTmp.Rows(0).Item("So_Ro").ToString.Trim.Replace("#", "")
        If _So_Ro = "" Then Exit Sub
        Dim strAddParar As String = "1#HDS#C#10#TM/CK#" + _So_Ro + "#"
        Dim _Process As Process = CyberSmlib.V_CallRun("Cyber.Input.dll", Me.Para, strAddParar)
        If Not _Process Is Nothing Then ProccessList.Add(_Process)
    End Sub
#End Region
...
#Region "Lenh/SO/Po/Ro/Vt"
    ...
    Private Sub GetDetail()
        ...
        Dim _Ma_TT As String = CbbMa_TT.SelectedValue.ToString.Trim
        Dim _Ma_Xe As String = TxtMa_Xe.Text.Trim

        Dim DsSo_Lenh As DataSet
        DsSo_Lenh = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPT5Detail", _Ma_TT + "#" + TxtLenh_Ro.Text.Trim + "#" + TxtMa_Xe.Text + "#" + _Ngay_CT.ToString("yyyyMMdd") + "#" + _Ngay_LCT.ToString("yyyyMMdd") + "#" + _
            txtSo_ct.Text.Trim + "#" + TxtMa_Kh.Text + "#" + TxtTen_kh.Text + "#" + txtOng_ba.Text + "#" + _
        ...
        If DsSo_Lenh.Tables(1).Columns.Contains("so_ro_hang") Then TxtSo_Ro_Hang.Text = DsSo_Lenh.Tables(1).Rows(0).Item("so_ro_hang")
        ...
        UpdateList()
    End Sub
    Private Sub V_Get_TT()
        TxtT_I.Double = 0
        TxtT_C.Double = 0
        TxtT_W.Double = 0
        TxtT_N.Double = 0
        TxtT_TT_I_C_W_N.Double = 0

        Dim _Lenh_Ro As String = TxtLenh_Ro.Text.Trim
        If _Lenh_Ro = "" Then Exit Sub
        Dim _TbName As String = "dbo.PH89"
        If _Lenh_Ro.Contains("H") Then _TbName = "dbo.PHHDI"

        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_SysExecute", "SELECT TOP 1 T_I,T_C,T_N,T_W,T_TT FROM " + _TbName.Trim + " WHERE So_RO = N'" + _Lenh_Ro + "'#" + M_Ma_Dvcs + "#" + M_User_Name)
        If DsTmp.Tables(0).Rows.Count = 0 Then
            DsTmp.Dispose()
            Exit Sub
        End If
        If DsTmp.Tables(0).Columns.Contains("T_I") Then TxtT_I.Double = DsTmp.Tables(0).Rows(0).Item("T_I")
        If DsTmp.Tables(0).Columns.Contains("T_C") Then TxtT_C.Double = DsTmp.Tables(0).Rows(0).Item("T_C")
        If DsTmp.Tables(0).Columns.Contains("T_W") Then TxtT_W.Double = DsTmp.Tables(0).Rows(0).Item("T_W")
        If DsTmp.Tables(0).Columns.Contains("T_N") Then TxtT_N.Double = DsTmp.Tables(0).Rows(0).Item("T_N")
        If DsTmp.Tables(0).Columns.Contains("T_TT") Then TxtT_TT_I_C_W_N.Double = DsTmp.Tables(0).Rows(0).Item("T_TT")
        DsTmp.Dispose()
    End Sub
...
#Region "Button"
    ...
    Private Sub V_New(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If TxtTk.Text.Trim = "" Then
            TxtTk.Text = M_Tk_No
            Txtten_Tk.Text = M_Ten_Tk_No
        End If
        V_New()
        Try
            CbbLoai_Thu.SelectedValue = M_Loai_Thu
        Catch ex As Exception
    ...
    Private Sub V_New()
        If Dt_DetailTmp.Rows.Count < 1 Then
            ...
            If Not M_Tk_Co.Trim = "" Then
                DrCurren.Item("Tk_i") = M_Tk_Co.Trim
                If DrCurren.Table.Columns.Contains("Ten_Tk_i") Then DrCurren.Item("Ten_Tk_i") = M_Ten_Tk_Co.Trim
            End If
...
#Region "Add Or Detete Item"
    ...
    Private Sub V_AddItem(Optional ByVal iRow As Integer = -1)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        '------Khong cho them dong khi ma_Gd =1: Thu theo hoa don
        If CbbMa_GD.SelectedValue.ToString.Trim.ToUpper = "1" Then Exit Sub
