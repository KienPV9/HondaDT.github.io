...
#Region "Khai bao bien Property----------------------"
    ...
    Dim _Para_So_Ro_Hang As String = ""
...
#End Region
    Private Sub PXK_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_System()
        ...
        AddHandler CmdTao_NhapKho.Click, AddressOf V_NhapKho

    End Sub
    Dim ProccessList As New Collection
    Private Sub V_NhapKho(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If (M_Mode = "M" Or M_Mode = "S") Then Exit Sub

        If Not Dt_DetailTmp.Columns.Contains("So_ct") Then Exit Sub
        Dim _So_ct As String = Dt_DetailTmp.Rows(0).Item("So_ct").ToString.Trim.Replace("#", "")
        If _So_ct = "" Then Exit Sub
        Dim strAddParar As String = "1#PNA#" + _So_ct + "#"
...
#Region "Lenh/SO/Po/Ro/Vt"
    ...
    Private Sub GetDetail()
        'If Dt_DetailTmp.Select("Ma_VT <> ''").Length > 0 Then
        '    Exit Sub
        'End If
        ...
        DsTmpDetal = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPXKDetail", TxtLenh_Ro.Text.Trim + "#" + dt.ToString("yyyyMMdd") + "#" + _
            M_Ma_CT.Trim + "#" + txtSo_ct.Text + "#" + CbbMa_Post.SelectedValue.Trim + "#" + M_Stt_Rec.Trim + "#" + M_Mode + "#" + _
             TxtMa_Xe.Text + "#" + TxtMa_Kh.Text + "#" + TxtDia_Chi.Text + "#" + txtOng_ba.Text + "#" + M_Ma_Dvcs.Trim + "#" + M_User_Name)
        ...
        If DsTmpDetal.Tables(0).Rows(0).Item("Status").ToString.Trim = "N" Then
            If DsTmpDetal.Tables(0).Rows(0).Item("Msg").ToString.Trim = "Y" Then
                MsgBox(DsTmpDetal.Tables(0).Rows(0).Item("Note").ToString.Trim, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, oSysVar("M_Cyber_Ver"))
            End If
            DsTmpDetal.Dispose()
            Exit Sub
        End If

        If DsTmpDetal.Tables(1).Rows.Count < 1 Then
            DsTmpDetal.Dispose()
            Exit Sub
        End If
        Dt_DetailTmp.Clear()
        'CyberSmodb.DeleteDatatable(Dt_DetailTmp, "Stt_Rec = '" + M_Stt_Rec + "'")
        'CyberSmodb.SQLDeleteTb(tbDetailTmp, "Stt_Rec", M_Stt_Rec.Trim)
        CyberSmodb.SQLTbToTb(DsTmpDetal.Tables(1), Dt_DetailTmp)
        ''
        If DsTmpDetal.Tables(2).Columns.Contains("Ma_kh") Then TxtMa_Kh.Text = DsTmpDetal.Tables(2).Rows(0).Item("Ma_kh")
        If DsTmpDetal.Tables(2).Columns.Contains("So_Ro_Hang") Then TxtSo_Ro_Hang.Text = DsTmpDetal.Tables(2).Rows(0).Item("So_Ro_Hang")

        If DsTmpDetal.Tables(2).Columns.Contains("Ten_kh") Then TxtTen_kh.Text = DsTmpDetal.Tables(2).Rows(0).Item("Ten_kh")
        If DsTmpDetal.Tables(2).Columns.Contains("Dia_Chi") Then TxtDia_Chi.Text = DsTmpDetal.Tables(2).Rows(0).Item("Dia_Chi")
        If DsTmpDetal.Tables(2).Columns.Contains("Ong_Ba") Then txtOng_ba.Text = DsTmpDetal.Tables(2).Rows(0).Item("Ong_Ba")
        If DsTmpDetal.Tables(2).Columns.Contains("Ma_Xe") Then TxtMa_Xe.Text = DsTmpDetal.Tables(2).Rows(0).Item("Ma_Xe")
        If DsTmpDetal.Tables(2).Columns.Contains("So_Ct") Then txtSo_ct.Text = DsTmpDetal.Tables(2).Rows(0).Item("So_Ct")
        DsTmpDetal.Dispose()
    ...
    Private Sub V_GetDetail(ByVal _So As String)
        ...
        TxtMa_Xe.Text = DsTmp.Tables(2).Rows(0).Item("Ma_xe")
...
#Region "Mainform"
    Private Sub V_MainSystem()
        ...
        Dim mnItemsF5 As New MenuItem("Chọn phiếu nhập/Phiếu xuất", AddressOf V_ShortF5, Keys.F5)
...
#Region "Button"
    ...
    Private Sub V_Save_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not CyberSupport.V_ChkExistNovoucher(AppConn, osysvar, M_LAN, DrDmct, M_Mode, M_Stt_Rec, M_Ma_CT, txtSo_ct.Text.Trim, _Ngay_Ct, _Ngay_LCt, M_Ma_Dvcs, M_User_Name, CyberSmlib) Then Exit Sub '---Kiem tra trung so chung tu
        '----------------------------------------------------------------------------------------------------------------------
        If Not ChkDa_Xuat(M_Mode) Then Exit Sub
        '----------------------------------------------------------------------------------------------------------------------
        ...
        CyberSmlib.FlushMemorySave()
    End Sub
    Private Function ChkDa_Xuat(ByVal _Mode) As Boolean
        ...
        Dim strSo_Ro As String = ""
        Dim strMa_vt As String = ""
        Dim strMa_kho As String = ""
        Dim strSo_Luong As String = ""

        For i As Integer = 0 To Dt_DetailTmp.Rows.Count - 1
            strSo_Ro = strSo_Ro & IIf(i = 0, "", "!~!") & Dt_DetailTmp.Rows(i).Item("So_Ro").ToString.Trim
        ...
        Next
        Dim _So_Ro As String = TxtLenh_Ro.Text
        DsTmp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ChkPXKDaXuat", nCount.ToString.Trim & "#" & _Mode & "#" & _Dt.ToString("yyyyMMdd") & "#" & _
                                                   _So_Ro & "#" & strSo_Ro & "#" & strMa_vt & "#" & strMa_kho & "#" & _
                                                   strSo_Luong & "#" & M_Stt_Rec & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Ma_VT").ToString.Trim = "" Or Dt_DetailTmp.Rows(iRow).Item("So_luong").ToString.Trim = 0 Then
        ...
        For iRow = 0 To Dt_DetailTmp.Rows.Count - 1
            ...
            Dt_DetailTmp.Rows(iRow).Item("So_ro") = TxtLenh_Ro.Text
            ...
            Dt_DetailTmp.Rows(iRow).Item("So_Ro") = TxtLenh_Ro.Text.Trim
    ...
    Private Sub V_Delete(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        For iRow As Integer = 0 To Dv_Master.Count - 1
            ...
            End If
        Next
        If Not ChkDa_Xuat("D") Then Exit Sub
