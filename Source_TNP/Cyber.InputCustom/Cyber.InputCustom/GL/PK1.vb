...
#Region "Columns Edit"
    ...
    Dim EditTen_Tk_No, EditTen_Tk_Co, EditTen_KH_N, EditTen_KH_C, EditMa_HD_C, EditSo_DN_TT_I, EditMA_HD_C_CT As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    Private Sub PK1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Load And Set And Default"
    ...
    Private Sub V_GetColumn()
        ...
        EditMa_HD_C.GetColumn(DetailGRV, "Ma_HD_C")
        ...
        EditSo_DN_TT_I.GetColumn(DetailGRV, "So_DN_TT_I")
        EditMA_HD_C_CT.GetColumn(DetailGRV, "MA_HD_C_CT")
    End Sub
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        EditMa_HD_C.V_ActiLookUpColumn(AddressOf V_Ma_HD_C, AddressOf L_Ma_HD_C)
        ...
        EditMA_HD_C_CT.V_ActiLookUpColumn(AddressOf V_Ma_HD_C_CT, AddressOf L_Ma_HD_C_CT)
    ...
    Private Sub VisibleColumnMa_GD()
        If CbbMa_GD.SelectedValue.ToString.Trim = "6" Then
            LblSo_DN_TT.Visible = True
            TxtSo_DN_TT.Visible = True
            CmdDanh_Sach_DN.Visible = True
            EditSo_DN_TT_I.Visible = True
        Else
            LblSo_DN_TT.Visible = False
            TxtSo_DN_TT.Visible = False
            CmdDanh_Sach_DN.Visible = False
            EditSo_DN_TT_I.Visible = False
        End If
...
#Region "VAT"
    ...
    Private Sub V_AddVat(ByRef _DtVat As DataTable, ByVal _Ma_Post As String, ByVal _Ngay_Ct As Date, ByVal _Ngay_LCt As Date, ByVal _Ma_NT As String, ByVal _Ty_GIa As Decimal)
        ...
        'If nCountSo_Detail > 0 Then _Tk_Thue_No = Dv_DetailTmp.Item(0).Item("TK_NO")
        ...
        If nCountSo_Detail = nCountSo_Hd_Gtgt And nCountSo_Hd_Gtgt > 1 Then
            For iRow As Integer = 0 To nCountSo_Detail - 1
                ...
                _Tk_Thue_No = Dt_DetailTmp.Rows(iRow).Item("Tk_NO")
...
#Region "danh sách phiếu đề nghị"
    Private Sub V_Danh_sach_DN(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        str = CyberSupport.V_GetStrSelectList(AppConn, osysvar, M_Para, "CP_GetListLDN4PK1", M_Ma_Dvcs & "#" & M_User_Name, M_LAN)
...
#Region "Lenh/SO/Po/Ro/Vt"
    Private Sub V_So_DN(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDataPK1FromLDN", So_DN & "#" & M_Stt_Rec & "#" & M_Ma_CT & "#" & Dt1.ToString("yyyyMMdd") & "#" & M_Mode & "#" & M_Ma_Dvcs & "#" & M_User_Name & "#" & TxtMa_Dnchi.Text)
...
#Region "Valid - Detail - Add On"
    ...
    Private Sub V_Ma_HD_i(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        CyberSupport.V_Lookup("Ma_HD", "DMHD", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_HD_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If drReturn Is Nothing Then
            Dt_DetailTmp.Rows(iRows)("Ma_HD_i") = ""
            Dt_DetailTmp.Rows(iRows)("Ten_HD_N") = ""
            Dt_DetailTmp.AcceptChanges()
            Exit Sub
        End If
        Dt_DetailTmp.Rows(iRows)("Ma_HD_I") = drReturn("Ma_HD").ToString.Trim
        Dt_DetailTmp.Rows(iRows)("Ten_HD_N") = drReturn("Ten_HD").ToString.Trim
        Dt_DetailTmp.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
        V_GetMa_td3_N(iRows)
    End Sub
    Private Sub V_Ma_HD_C(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        'CyberSupport.V_Ma_Hd_i(M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
        CyberSupport.V_Lookup("Ma_HD", "DMHD", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_HD_C(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'CyberSupport.L_Ma_Hd_i(M_Mode, M_Ma_CT, sender, DetailGRV)
        Dim iRows As Integer = -1
        Dim drReturn As DataRow
        iRows = DetailGRV.FocusedRowHandle
        ...
        drReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        If drReturn Is Nothing Then
            Dt_DetailTmp.Rows(iRows)("Ma_HD_C") = ""
            Dt_DetailTmp.Rows(iRows)("Ten_HD_C") = ""
            Dt_DetailTmp.AcceptChanges()
            Exit Sub
        End If
        Dt_DetailTmp.Rows(iRows)("Ma_HD_C") = drReturn("Ma_HD").ToString.Trim
        Dt_DetailTmp.Rows(iRows)("Ten_HD_C") = drReturn("Ten_HD").ToString.Trim
        Dt_DetailTmp.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
        V_GetMa_td3_C(iRows)
    End Sub
    Private Sub V_Ma_HD_C_CT(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_td3", "dmtd3", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_HD_C_CT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim iRows As Integer = -1
        Dim drReturn As DataRow
        iRows = DetailGRV.FocusedRowHandle
        If iRows < 0 Then Exit Sub
        drReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        If drReturn Is Nothing Then
            Dt_DetailTmp.Rows(iRows)("MA_HD_C_CT") = ""
            Dt_DetailTmp.AcceptChanges()
            Exit Sub
        End If
        Dt_DetailTmp.Rows(iRows)("MA_HD_C_CT") = drReturn("Ma_td3").ToString.Trim
        Dt_DetailTmp.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
    End Sub
...
#End Region
#Region "get ma_td3"
    Private Sub V_GetMa_td3_N(ByVal iRow As Integer)
        M_Mode = M_Mode.Trim
        If iRow < 0 Then Exit Sub
        If iRow < 0 Then Exit Sub
        Dim _Ma_HD As String = If(Dt_DetailTmp.Columns.Contains("MA_HD_I"), Dv_DetailTmp.Item(iRow).Item("MA_HD_I").ToString.Trim, "")
        Dim DsMa_td3 As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GETMa_td3", _Ma_HD & "#" & M_Ma_Dvcs & "#" & M_User_Name)

        Dt_DetailTmp.Rows(iRow).BeginEdit()
        Dt_DetailTmp.Rows(iRow).Item("MA_TD3_I") = DsMa_td3.Tables(0).Rows(0).Item("ma_td3")
        Dt_DetailTmp.Rows(iRow).EndEdit()
    End Sub
    Private Sub V_GetMa_td3_C(ByVal iRow As Integer)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If iRow < 0 Then Exit Sub
        Dim _Ma_HD As String = If(Dt_DetailTmp.Columns.Contains("MA_HD_C"), Dv_DetailTmp.Item(iRow).Item("MA_HD_C").ToString.Trim, "")
        Dim DsMa_td3 As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GETMa_td3", _Ma_HD & "#" & M_Ma_Dvcs & "#" & M_User_Name)

        Dt_DetailTmp.Rows(iRow).BeginEdit()
        Dt_DetailTmp.Rows(iRow).Item("MA_HD_C_CT") = DsMa_td3.Tables(0).Rows(0).Item("ma_td3")
        Dt_DetailTmp.Rows(iRow).EndEdit()
    End Sub
#End Region
#End Region
