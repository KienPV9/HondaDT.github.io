...
#Region "Columns Edit"
    ...
    Dim EditTen_Vt, EditDvt, EditSL_Ton, EditSL_RO, EditSL_Xuat, EditSL_Da_Xuat, EditSL_Xuat_Pick, EditStt_rec0, EditSL_Da_Xuat_Pick, EditSL_CL, EditGia2, EditThue, EditTien2, EditSo_RO, Editma_cv, EditTen_cv3, Editma_ctcv, EditGhi_chu, Editso_ct0, Editngay_ct0, Editma_ct, EditFtag As New Cyber.Fill.CyberColumnGridView
...
#End Region
    ...
    Private Sub V_LoadRo(ByVal _Load As String, ByVal _Load1 As String)
        ...
        Dim DsTmpRO As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetROSNKDetail1", _Load + "#" + _Load1 + "#" + M_Ma_Dvcs.Trim + "#" + M_User_Name)
    ...
    Private Sub V_Loaddatatabse(ByVal _Status As String, ByVal _So_Ro As String)
        ...
        Dim Dt1 As Date = TxtNgay_Ct1.Value
        Dim Dt2 As Date = Txtngay_ct2.Value
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetROSNKDetail", _Status + "#" + _So_Ro + "#" + _Pick + "#" + Dt1.ToString("yyyyMMdd") + "#" + Dt2.ToString("yyyyMMdd") + "#" + IIf(Chkis_Hd.Checked, "1", "0") + "#" + M_Ma_Dvcs.Trim + "#" + M_User_Name)
        ...
        If DsTmp.Tables(0).Columns.Contains("SO_NK") Then TxtSo_NK.Text = DsTmp.Tables(0).Rows(0).Item("SO_NK")
    ...
    Private Function Tao_Px(ByVal _Loai As String, ByVal iRow As Integer) As String
        ...
        For i = j To nCount
            ...
            _StrSave = _StrSave + ";INSERT KTVKTVKTV SELECT '" & Dv_Ro.Item(i).Item("So_RO").ToString.Trim & "',N'" & Dv_Ro.Item(i).Item("Ftag").ToString.Trim & "',N'" & Dv_Ro.Item(i).Item("SO_CT0").ToString.Trim & "',N'" & Dv_Ro.Item(i).Item("Ma_ct").ToString.Trim & "'"
        Next
        If _StrSave = "" Then Return ""
        Dim Dt3 As Date = Txtngay_HD.Value
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ApprolNK", _Loai + "#" + _StrSave & "#" & TxtSo_Hd.Text & "#" & Dt3.ToString("yyyyMMdd") & "#" & TxtSo_Ro.Text & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Private Sub V_danh_sach(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetROSNKDetail_Search", _status & "#" & Dt1.ToString("yyyyMMdd") & "#" & Dt2.ToString("yyyyMMdd") & "#" & TxtMa_KH.Text.ToString.Trim & "#" & TxtSo_NK.Text.ToString.Trim & "#" & IIf(Chkis_Hd.Checked, "1", "0") & "#" & M_Ma_Dvcs.Trim & "#" & M_User_Name)
        ...
        CyberSmodb.SQLTbToTb(DsTmp.Tables(1), Dt_Ro)
        If DsTmp.Tables(0).Columns.Contains("Ma_KH") Then TxtSo_Ro.Text = DsTmp.Tables(0).Rows(0).Item("Ma_KH")
        Dt_Ro.AcceptChanges()
        CyberSupport.V_MsgChk(DsTmp.Tables(0), Me.Sysvar, M_LAN)
        DsTmp.Dispose()
    ...
    Private Sub V_Update(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If TxtSo_Hd.Text = "" Then
            If MsgBox("Số hóa đơn đang để trống, bạn có muốn tiếp tục không ?", MsgBoxStyle.YesNo + MsgBoxStyle.Critical, "Chỉnh sửa nhập kho") = MsgBoxResult.Yes Then
                V_CongViec(sender, e)
            Else
                Exit Sub
            End If
